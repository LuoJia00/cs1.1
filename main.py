...
def patch_windows_proactor_accept_reset():
    """Keep the Windows asyncio listener alive after an early client reset.

    Python's Proactor event loop closes the listening socket when AcceptEx
    reports ERROR_NETNAME_DELETED (WinError 64). Browsers can cause this by
    opening and cancelling a connection before the server accepts it. The
    process then stays alive but port 3000 refuses every new connection.

    This is a local backport of the upstream recovery behaviour: retry only
    the known transient client-reset errors and preserve the standard fatal
    error path for every other OSError. Proactor remains enabled because the
    Codex/GPT Image helpers require asyncio subprocess support on Windows.
    """
    ...
    loop_class = proactor_events.BaseProactorEventLoop
    if getattr(loop_class, "_canvas_accept_reset_patch", False):
        return True

    transient_winerrors = {64, 1236}  # ERROR_NETNAME_DELETED / ERROR_CONNECTION_ABORTED

    def start_serving(self, protocol_factory, sock, sslcontext=None,
                      server=None, backlog=100, ssl_handshake_timeout=None):
        def accept_loop(future=None):
            try:
                ...
                if (winerror in transient_winerrors and
                        sock.fileno() != -1 and not self.is_closed()):
                    # The reset belongs to the abandoned client connection,
                    # not the listening socket. Schedule a fresh AcceptEx.
            ...
            future.add_done_callback(accept_loop)

        self.call_soon(accept_loop)

    loop_class._start_serving = start_serving
    loop_class._canvas_accept_reset_patch = True
    return True


def install_windows_proactor_exception_filter(loop):
    """Hide only the duplicate task warning emitted for recovered resets."""
    if sys.platform != "win32" or getattr(loop, "_canvas_accept_reset_filter", False):
        return
    ...
    def exception_handler(current_loop, context):
        ...
        if (isinstance(exc, OSError) and
                getattr(exc, "winerror", None) in {64, 1236} and
    ...
    loop._canvas_accept_reset_filter = True
...
# This maintained copy is updated through Git, including tools and dependencies.
# The original downloader only replaces main.py/static and can erase local work.
CODE_UPDATE_MESSAGE = "此修改版通过 GitHub Desktop 更新：关闭服务后，先提交本机代码修改，再 Fetch origin / Pull origin，完成后重新启动。原作者在线覆盖更新已停用。"
GITHUB_REPO_URL = "https://github.com/LuoJia00/cs1.1"
GITHUB_VERSION_URL = "https://raw.githubusercontent.com/LuoJia00/cs1.1/main/VERSION"
GITHUB_TREE_URL = "https://api.github.com/repos/LuoJia00/cs1.1/git/trees/main?recursive=1"
GITHUB_RAW_ROOT = "https://raw.githubusercontent.com/LuoJia00/cs1.1/main"
...
async def startup_event():
    ...
    install_windows_proactor_exception_filter(GLOBAL_LOOP)
...
def app_info():
    ...
    return {
        "version": version,
        "update_mode": "git",
        "update_message": CODE_UPDATE_MESSAGE,
...
def check_update():
    """Maintained copies use Git for updates, never the upstream overwrite flow."""
    return {
        "current": current_app_version(), "update_mode": "git",
        "message": CODE_UPDATE_MESSAGE, "update_available": False,
        "reachable": False, "latest": {},
    }

def check_upstream_update():
    """Original upstream checker, retained for reference; not an application route."""
...
def update_from_github(req: UpdateRequest = UpdateRequest()):
    raise HTTPException(status_code=409, detail=CODE_UPDATE_MESSAGE)

def update_from_upstream(req: UpdateRequest = UpdateRequest()):
    """Original downloader, retained for reference; not an application route."""
...
async def run_captured_subprocess(args, cwd=None, input_data=None, timeout=None):
    """Run a captured child process with a Windows pipe fallback.

    Some restricted Windows sessions allow CreateProcess but reject the named
    overlapped pipes used by asyncio with WinError 5. Normal anonymous pipes
    still work, so fall back to subprocess.run in a worker thread only for
    ...
    try:
        ...
        try:
            completed = await asyncio.to_thread(run_with_standard_pipes)
...
async def run_codex_cli(prompt, model="", image_paths=None, timeout=None, output_last_message=True):
    ...
    try:
        returncode, stdout, stderr = await run_captured_subprocess(
            args,
            cwd=BASE_DIR,
    ...
    if last_path and os.path.exists(last_path):
        ...
        try:
            ...
            pass
    if returncode != 0:
        message = err_text or out_text or last_text or f"exit={returncode}"
...
def gpt_image_2_skill_model_arg(model="", provider="openai"):
    ...
    if provider == "codex":
        if not value or low.startswith("$imagegen") or low.startswith("gpt-image"):
            # This is the Responses executor, not the image model. Reuse the
            # app's Codex chat default; gpt-5.4 is rejected by ChatGPT accounts.
...
def gpt_image_2_skill_size_arg(size="", model="", prompt="", provider="openai"):
    ...
    if str(provider or "").strip().lower() == "codex":
        ...
        if width and height:
            # The helper's 4K alias means 3840x2160, even for square/portrait
            # requests. Send dimensions so the requested composition survives.
...
def gpt_image_2_skill_prompt_arg(prompt="", size="", provider="openai"):
    ...
    if width and height:
        ...
        size_arg = "1K" if long_edge < 1800 else ("2K" if long_edge < 3000 else "4K")
    ratio_text = ""
    if width and height:
...
def gpt_image_2_skill_error_messages(value, depth=0):
    """Unwrap the helper's JSON-encoded HTTP body, retaining the actual cause."""
    ...
    if isinstance(value, str):
        ...
        if isinstance(parsed, (dict, list)):
            return gpt_image_2_skill_error_messages(parsed, depth + 1)
        return [value]
    if isinstance(value, list):
        return [message for item in value for message in gpt_image_2_skill_error_messages(item, depth + 1)]
    if isinstance(value, dict):
        messages = []
        for key in ("message", "detail", "error"):
            messages.extend(gpt_image_2_skill_error_messages(value.get(key), depth + 1))
        if not messages and value.get("code"):
            messages.append(str(value["code"]))
        return messages
    return []
...
def gpt_image_2_skill_failure_message(stdout_text="", stderr_text="", returncode=0):
    ...
    for item in objects:
        ...
        messages.extend(gpt_image_2_skill_error_messages(error))
...
async def generate_codex_provider_image_via_gpt_image_2_skill(prompt, size, model, ref_paths=None):
    ...
    for attempt_index, (attempt_provider_args, attempt_provider) in enumerate(attempts):
        ...
        out_text, err_text = codex_decode_output(stdout, stderr)
        if returncode != 0:
            message = gpt_image_2_skill_failure_message(out_text, err_text, returncode)
...
async def codex_status():
    ...
    try:
        ...
        return {
            ...
            "raw": {"stdout": out_text, "stderr": err_text, "returncode": returncode},
        }
    except Exception as exc:
        return {
            "installed": False,
...
if __name__ == "__main__":
    import uvicorn
    patch_windows_proactor_accept_reset()
