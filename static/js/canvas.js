...
function renderNode(node){
    ...
    // Workflow JSON may provide a descriptive name so imported multi-node flows
    // remain understandable at a glance. Existing nodes keep their old titles.
    const displayTitle = String(node.name || '').trim()
        || (node.type === 'image' && node.url ? nodeTitleForMedia(node) : title);
